...
var MisproTransformer = require('./parsers/misproParser');
...
Mispro.prototype.preprocess = function (inputFile, cb) {
...
Mispro.prototype.misproDetectionStream = function() {
    debug('Running mispronunciation detection on', this.outputDir);
    var command = [this.settings.misproDetectionScript, this.outputDir].join(' ');
    var child = spawn(this.settings.misproDetectionScript, [this.outputDir]);

    return child.stdout;
};


Mispro.prototype.getMisproResultsStream = function (misproOutput) {
    // mispro output is a stream
    debug('Transforming mispronunciation results');
    var transformer = MisproTransformer();
    misproOutput.pipe(split()).pipe(transformer)
    return transformer;
};

Mispro.prototype._initDirAsync = Promise.promisify(Mispro.prototype._initDir);
Mispro.prototype.preprocessAsync = Promise.promisify(Mispro.prototype.preprocess);
